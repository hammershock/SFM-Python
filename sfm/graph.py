...
def initial_register(G, K):
    ...
    for n, match in enumerate(edge_data['matches']):
        if edge_data['mask_enough_tracks'][n] and edge_data["mask_inliers"][n]:
            assert isinstance(match, cv2.DMatch)
            i, j = match.queryIdx, match.trainIdx
            for node_idx, feat_idx in edge_data["tracks"][(i, j)]:
                G.nodes[node_idx]["constructed"][feat_idx] = n
...
def apply_increment(G, K, X3d, increment_mask=None):
    ...
    for n, match in enumerate(edge_data['matches']):
        if edge_data['mask_enough_tracks'][n] and edge_data["mask_inliers"][n]:
            ...
            i, j = match.queryIdx, match.trainIdx
            for node_idx, feat_idx in edge_data["tracks"][(i, j)]:
                if feat_idx not in G.nodes[node_idx]["constructed"]:
                    G.nodes[node_idx]["constructed"][feat_idx] = k
                    k += 1
